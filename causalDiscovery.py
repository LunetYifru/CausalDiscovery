...
def preparedata(file, target):
    ...
    df_data = df_data.drop(columns = ['Time','LABEL_ERROR_STATE'])
    ...
    scaled_df = pd.DataFrame(scaled, columns=df_data.columns)
    df_data = scaled_df
    
    # Adding the "True cause" column for traget
    targetidx = df_data.columns.get_loc(target)
    TrueCause = []
    for i in range(2,len(df_data)):
        TrueCause.append(df_data.iloc[:,targetidx][i]*2)
        
    TrueCause.extend([0,0])  
    df_data['True Cause'] = TrueCause
    print(df_data.head())
    
    
    #Correlation between features
    print('----------Correlations------------')
    corr_matrix = df_data.corr(method ='spearman')
    ...
    df_dataTr = df_data.transpose()
    model = KMeans(k=2, maxiter =300, distance='cosine', record_heterogeneity=[], verbose=True, seed=123)
    centroids, cluster_assignment = model.fit(df_dataTr)
    
    print('Cluster Assignment: ',cluster_assignment)
    
    same_cluster = []
    same_cluster_idx = []
    for l in range(df_data.shape[1]):
        if cluster_assignment[targetidx] == cluster_assignment[l]:
            same_cluster.append(df_data.columns[l])
            same_cluster_idx.append(l)
        
    print('Same Cluster', same_cluster)
    # model.plot_heterogeneity()
    # plt.show()
    ...
    return x, y, same_cluster_idx, df_data
...
def findcauses(target, cuda, epochs, kernel_size, layers, 
    ...
    """Discovers potential causes of one target time series(correlations), validates these potential causes with PIVM and discovers the corresponding time delays"""
    
    print("\n", "Analysis started for target: ", target)
    torch.manual_seed(seed)
    
    X_train, Y_train, same_cluster_idx, df_data = preparedata(file, target) 
    ...
    print('Sorted attention scores --->',s)
    ...
    if len(s)<=5:
        ...
        # potentials = same_cluster_idx
        
        
    print("Potential causes: ", potentials, same_cluster_idx)
    ...
    for idx in potentials:
        ...
        print('VALIDATIONNNN ---->',testdiff, diff)

        #if testdiff>(diff*significance): 
        if testdiff<(diff*1.2): 
